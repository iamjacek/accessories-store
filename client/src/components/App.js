  ...
  return (
    ...
    <main className="container mx-auto px-4 sm:px-8 flex-grow max-w-full">
      ...
      <div className="flex flex-col justify-center mb-4 text-center">
        ...
        <div
          className="flex flex-wrap justify-center mb-6 mx-auto"
          style={{ maxWidth: "1920px" }}
        >
          {filteredCats(searchTerm, data.cats).map((cat) => (
            <Link
              className="w-48 rounded overflow-hidden shadow-lg my-8 mx-3 text-left hover:shadow-2xl transition-shadow duration-200"
              key={cat._id}
              to={`/product/${cat._id}`}
            >
              <img
              ...
              <div className="px-4 py-2 border-top">
                <div className="flex flex-col justify-between content-center">
                  <h2 className="text-md text-gray-700 font-semibold mb-0 pb-1 leading-7 text-shadow">
                    {" "}
                    {cat.name}{" "}
                  </h2>
